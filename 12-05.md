...
- Таблицы  rental, inventory, film  их можно убрать из запроса,   
Условие where лучше переписать как join, 
По поводу date(p.payment_date) -  лучше создать  индексы отдельно для для p.payment_date; p.amount, а также c.last_name и c.first_name.
